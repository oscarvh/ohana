...
- Tailwind v4 is compiled at build time with `@tailwindcss/cli` (installed as devDependency): `npm run css` generates `src/assets/main.css` (minified, ~33 KB, committed) and is chained into both `build` and `dev` scripts. There is NO browser-CDN Tailwind anymore — don't add one; no `tailwind.config.js` either (v4 is CSS-first).
- Design tokens (colors, fonts, text sizes, spacing) live ONLY in the `@theme { ... }` block inside `src/css/style.css` (the Stitch "Cancionero Ohana" design system). `src/editar/index.html` links `../assets/main.css` (relative, works on localhost and Pages). Headings use `font-serif` (Playfair Display), body `font-sans` (Plus Jakarta Sans); icons are Material Symbols Outlined. Use the token-based utilities (`bg-surface-container-lowest`, `text-headline-md`, `p-space-md`, ...) rather than arbitrary values.
- Icon font is subset via `icon_names=` in the Google Fonts URL (base.njk + editar/index.html) — 21 icons ≈ 23 KB instead of the 4 MB full variable font. When adding a new Material Symbol (including ones set from JS like atril.js does), add its name to the `icon_names` param in BOTH font links.
- Atril/cancion text fonts: Playfair Display 600/700 (no italic loaded) + Plus Jakarta Sans 400/500/600/700, latin subsets served by Google Fonts.
...
- The `/editar/` page talks to the worker at a hardcoded `API` const in its inline script (now `src/editar/index.html`, no duplicated theme since it links the shared `../assets/main.css`).
